...
class SalesViewController: UIViewController {
    ...
    @IBAction func softdrinksTapped(_ sender: UIButton) {
        softdrinksArr.append("Softdrinks")
    ...
    func happyDeal() {
        if softdrinksArr.count >= 2 && snack150Arr.count >= 1 {
            inCartItems.append("Happy Deal")
            inCartPrice.append(5.50)
            softdrinksArr.removeLast()
    ...
    func sandwichDeal() {
        if softdrinksArr.count >= 1 && sandwichArr.count >= 1 {
    ...
    func longdrinkDeal() {
        if minisArr.count >= 1 && softdrinksArr.count >= 1 {
    ...
    func appendToItemsAndUpdate() {
        inCartItems.append(contentsOf: softdrinksArr)
        inCartItems.append(contentsOf: snack150Arr)
        inCartItems.append(contentsOf: sandwichArr)
        inCartItems.append(contentsOf: beerArr)
        inCartItems.append(contentsOf: wine6Arr)
        inCartItems.append(contentsOf: tapasArr)
        inCartItems.append(contentsOf: coffeeArr)
        inCartItems.append(contentsOf: muffinArr)
        inCartItems.append(contentsOf: minisArr)
        inCartPrice.append(contentsOf: softdrinksPriceArr)
        inCartPrice.append(contentsOf: snacksPriceArr)
        inCartPrice.append(contentsOf: sandwichPriceArr)
        inCartPrice.append(contentsOf: beerPriceArr)
        inCartPrice.append(contentsOf: wine6PriceArr)
        inCartPrice.append(contentsOf: tapasPriceArr)
        inCartPrice.append(contentsOf: coffeePriceArr)
        inCartPrice.append(contentsOf: muffinPriceArr)
        inCartPrice.append(contentsOf: minisPriceArr)
        softdrinksArr = []
        softdrinksPriceArr = []
        snack150Arr = []
        snacksPriceArr = []
